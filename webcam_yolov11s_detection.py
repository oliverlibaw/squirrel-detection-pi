#!/usr/bin/env python3
"""
YOLOV11s Object Detection with Webcam on Raspberry Pi
This script runs the custom YOLOV11s model on webcam input using the Hailo AI accelerator.
"""
...
class YOLOV11sWebcamDetector:
    ...
    def run_detection(self, camera_index=0, resolution=(640, 480)):
        ...
        try:
            ...
            print("Starting YOLOV11s detection on webcam...")
            print("Press 'q' to quit, 's' to save screenshot")
            
            # Create display window
            cv2.namedWindow("YOLOV11s Detection", cv2.WINDOW_NORMAL)
            cv2.resizeWindow("YOLOV11s Detection", resolution[0], resolution[1])
            ...
            while True:
                ...
                # Add FPS overlay
                self._add_fps_overlay(output_frame)
                
                # Add inference time
                inference_text = f"Inference: {inference_time*1000:.1f}ms"
                cv2.putText(output_frame, inference_text, (10, 120), 
                           cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
                
                # Add detection count (using filtered results)
                detection_count = len(filtered_results)
                count_text = f"Detections: {detection_count}"
                cv2.putText(output_frame, count_text, (10, 150), 
                           cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
                
                # Display the frame
                cv2.imshow("YOLOV11s Detection", output_frame)
                ...
                if key == ord('q'):
                    ...
                    filename = f"yolov11s_screenshot_{screenshot_counter}.jpg"
...
def main():
    parser = argparse.ArgumentParser(description="YOLOV11s Object Detection with Webcam")
    ...
    # Create detector
    detector = YOLOV11sWebcamDetector(
